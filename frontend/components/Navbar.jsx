...
const Navbar = () => {
    ...
    const toggleTheme = () => {
        const swtichTheme = !isDark

        setIsDark(swtichTheme)
        document.documentElement.classList.toggle("dark", swtichTheme);
        window.localStorage.setItem("theme", swtichTheme ? "dark" : "light")
    }
    return (
        <div className='w-full h-16 bg-white/70 dark:bg-white/[0.03] backdrop-blur-xl border-b border-slate-200/70 dark:border-white/[0.07] flex items-center px-6 gap-6 font-sans transition-colors duration-300'>
            ...
            <div className='flex items-center gap-2 shrink-0'>
                ...
                <div className=' relative ml-1'>
                    <button onClick={() => setMenuOpen(p => !p)} className=' flex items-center gap-2 pl-1.5 pr-2 h-10 rounded-lg hover:bg-slate-100 dark:hover:bg-white/[0.06] transition-colors duration-150'>
                        ...
                        <span className='text-[13.5px] font-medium text-slate-700 dark:text-slate-200 hidden sm:flex items-center gap-1'> {name}
                            <ChevronDown size={14} className={`text-slate-400 dark:text-slate-500 transition-transform duration-200 ${menuOpen ? "rotate-180" : ""}`} />
                        </span>
                    </button>
                    ...
                    {menuOpen && (
                        <div className='absolute right-0 mt-2 w-52 bg-white/95 dark:bg-[#12121c]/95 backdrop-blur-xl border border-slate-200 dark:border-white/[0.08] rounded-xl shadow-xl py-1.5 z-50 animate-[fadeIn_0.15s_ease-out]'>
                            ...
                            <button onClick={handleLogout} className='w-full flex items-center gap-2.5 px-3.5 py-2 text-[13px] text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors duration-150'>
                                <LogOut size={15} />
                                Log Out
                            </button>
                        </div>
